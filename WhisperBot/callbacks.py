	...
	if callback_query.data.lower() == "home":
		...
		if callback_query.from_user.id in data_list:
			...
			if q:
				...
				await callback_query.answer("لم يتم العثور على الرسالة", show_alert=True)
			SESSION.commit()
		else:
			await callback_query.answer("شبيك ملطلط هاي الهمسة مو الك !", show_alert=True)
